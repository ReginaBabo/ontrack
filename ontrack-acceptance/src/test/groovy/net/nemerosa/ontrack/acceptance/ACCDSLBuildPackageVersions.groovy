...
class ACCDSLBuildPackageVersions extends AbstractACCDSL {
    ...
    @Test
    void 'Uploading package versions for a build using a properties file referring to labels'() {
        // Project to use as reference
        def refName = uid('P')
        ontrack.project(refName) {
            setPackageIds(
            ...
            config {
                buildLinkDisplayOptions(true)
            }

            branch("master") {
                build("3.38.5-12334567") {
                    // Label to refer to
                    config {
                        label "3.38.5"
                    }
        ...
        // Reference build
        def ref = ontrack.build(refName, "master", "3.38.5-12334567")
        ...
        assert ontrackDep.target != null: "Ref build has been found"
        assert ontrackDep.target.id == ref.id
        assert ontrackDep.target.name == "3.38.5-12334567"
        def refBuild = new Build(ontrack, ontrackDep.target)
        assert refBuild.config.label == "3.38.5"
